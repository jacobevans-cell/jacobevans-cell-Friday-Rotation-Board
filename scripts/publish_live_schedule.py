...
# Trigger live publication after workflow creation.
p=Path('index.html')
...
regular_pairs={
 '2026-09-04':('Abby','Evans'),'2026-09-11':('Manier','Lingam'),'2026-09-25':('Abby','Evans'),
 '2026-10-02':('Manier','Lingam'),'2026-10-16':('Abby','Evans'),'2026-10-30':('Manier','Lingam'),
 '2026-11-06':('Abby','Evans'),'2026-11-20':('Manier','Lingam'),'2026-12-04':('Abby','Evans'),
 '2026-12-11':('Manier','Lingam'),'2026-12-18':('Abby','Evans'),'2027-01-08':('Manier','Lingam'),
 '2027-01-15':('Abby','Evans'),'2027-01-22':('Manier','Lingam'),'2027-02-05':('Abby','Evans'),
 '2027-02-12':('Manier','Lingam'),'2027-02-19':('Abby','Evans'),'2027-03-05':('Manier','Lingam'),
 '2027-03-19':('Abby','Evans'),'2027-04-02':('Manier','Lingam'),'2027-04-09':('Abby','Evans'),
 '2027-04-16':('Manier','Lingam'),'2027-04-30':('Abby','Evans'),'2027-05-07':('Manier','Lingam'),
...
for f in data['fridays']:
    ...
    if d < '2026-09-04':
        st=f.get('status',{})
        for name in STAFF: st.setdefault(name,'OFF')
        f['status']=st
        continue
    if typ=='REGULAR FRIDAY':
        pair=regular_pairs[d]
        if any((name,d) in off for name in pair): raise SystemExit(f'hard-off conflict in regular pair {d}: {pair}')
        f['morning1'],f['morning2']=pair; f['morningExtra']=''
        f['studentCoverage']='Assigned rotating teachers continue to 1:00'
        f['officeCoverage']='Jenny 7:30–11:30 | Cortni 9:30–1:30'; f['training']='—'
        ...
        if any((name,d) in off for name in STAFF): raise SystemExit(f'unhandled training-day hard off on {d}')
        f['morning1'],f['morning2']=pair; f['morningExtra']=''
        ...
        st[stay]='Full-Day Coverage — Misses Training'; st[join]='Morning Coverage + Training'; f['status']=st
    elif typ=='NO SCHOOL / NO SCHEDULE':
        f['morning1']=f['morning2']='—'; f['morningExtra']=''; f['studentCoverage']=f['officeCoverage']=f['training']='—'
...
months=data['months']; summary=[]
from datetime import datetime
for name in STAFF:
    month_counts={mo:0 for mo in months}; total=missed=0
    for f in data['fridays']:
        st=f.get('status',{}).get(name,'OFF')
        if st in working:
            total+=1; mo=datetime.strptime(f['date'],'%Y-%m-%d').strftime('%b %Y')
...
for f in data['fridays']:
    ...
    if d>='2026-09-04':
        text=json.dumps(f); assert 'Dorr' not in text and 'McKinley' not in text and 'Coach' not in text
    if d>='2026-09-04' and typ=='REGULAR FRIDAY': assert sum(st.get(n)=='Full Day 7:30–1:30' for n in ROTATORS)==2
